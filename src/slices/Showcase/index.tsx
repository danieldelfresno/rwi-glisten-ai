    ...
    >
      ...
      <AnimatedContent>
        <PrismicRichText field={slice.primary.heading} components={{
          heading2: ({ children }) => (
            <div className="text-5xl text-balance text-center font-medium">
              {children}
            </div>
          )
        }}
      ...
      <div className="grid mt-16 items-center rounded-xl border border-blue-50/20 bg-gradient-to-b from-slate-50/15 to-slate-50/5 px-8 py-8 backdrop-blur-sm lg:grid-cols-3 lg:py-12 gap-8 lg:gap-0">
        ...
        <PrismicNextImage field={slice.primary.image} className={clsx("opacity-90 shadow-2xl lg:col-span-2 lg:pt-0", slice.variation === 'default' ? "lg:-order-1 lg:translate-x-[-15%]" : "lg:order-1 lg:translate-x-[15%]")} />
      </div>
    </Bounded>
  );
};
